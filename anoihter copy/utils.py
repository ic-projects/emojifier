...
class TextLoader():
    ...
    def create_batches(self):
        ...
        #     print(xdata[i])
        #     print(ydata[i])
        #     before += unicode(self.chars[xdata[i]])
        #     after += unicode(self.chars[ydata[i]])
        # print(before)
        # print(after)
        ...
        characters = map(self.vocab.get,"1234567890 -_=+[];:'@#~,<.>/?|$^&*()abcdefghijklmnopqrstuvwxyz")
        for i in range (len(ydata)):
            if not (ydata[i] in characters): #is emoji
        ...
        for i in range (len(val_ydata)):
            if not (val_ydata[i] in characters): #is emoji
                for x in range(last + 1,i):
                    val_ydata[x] = val_ydata[i]
                last = i
