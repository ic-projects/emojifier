...
def qiang(message):
    ...
    global SIM_WEIGHT
    global EMOJINUMBER
    return supersample(message, model, tags, emojis, THRESHOLD, DECAY, SIM_WEIGHT, EMOJINUMBER)
...
def supersample(prime, model, tags, emojis, threshold, decay, sim_weight, emojinumber):
    prime = prime.strip()
    words = prime.split()
    return sample(words, model, tags, emojis, threshold, decay, sim_weight, emojinumber)


def sample(words, model, tags, emojis, threshold, decay, sim_weight, emojinumber):
    emojistoadd = []
    for i in range(len(words)):
        if (i == 0):
            values = getValue(words[i], model, tags, emojis, sim_weight, threshold)
        else:
            values = [x + y for x, y in zip(map(lambda x: x / decay, values),
                                            getValue(words[i], model, tags, emojis, sim_weight,
                                                     threshold))]
...
def getValue(lastword, model, tags, emojis, sim_weight, threshold):
    values = map(lambda x: similarity(lastword, x, model, sim_weight), tags)
    if np.max(values) == 0:
...
while True:
    ...
    if message[0:2] == "#T":
        THRESHOLD = float(message[2:])
    elif message[0:2] == "#D":
        DECAY = int(message[2:])
    elif message[0:2] == "#S":
        SIM_WEIGHT = float(message[2:])
    elif message[0:2] == "#N":
        EMOJINUMBER = int(message[2:])
    else:
        print(qiang(message))
